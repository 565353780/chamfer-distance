...
chamfer_KD_3DFunction::forward(torch::autograd::AutogradContext *ctx,
...
std::vector<torch::Tensor>
chamfer_KD_3DFunction::backward(torch::autograd::AutogradContext *ctx,
...
const std::vector<torch::Tensor> chamfer_cuda_kd(const torch::Tensor &input1,
  ...
  const std::vector<torch::Tensor> dists_with_idxs =
      chamfer_KD_3DFunction::apply(contiguous_input1, contiguous_input2);
